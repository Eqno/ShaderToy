...
{
	return light_position + vec3((rng.x - 0.5) * light_size_x, 0, (rng.y - 0.5) * light_size_y);
}
...
intersect_light(Ray ray)
{
	...
	vec3 p = ray_at(ray, t);
	if(all(lessThan(abs(light_position - p).xz, vec2(light_size_x, light_size_y)))) {
		return t;
	}
	...
	for(int i = 0; i < NUM_BOUNCES; i++) {
		...
		{ /* brdf */
			...
			if(albedo_next.a > 0.0) { /* hit light_source */
				...
				vec3 Le = light_albedo.rgb * light_albedo.a;
				contrib += tp * (Le  * brdf) / (light_pdf + brdf_pdf);
...
mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	light_size_x = 0.25;
    light_size_y = 0.25;
	light_area = light_size_x * light_size_y;
	light_position = vec3(0.5 * sin(iTime), 0.90, 0.5 * cos(iTime));
	light_normal = vec3(0, -1, 0);
	seed = 0;
	light_albedo = vec4(1, 1, 1, 4.0 / (light_size_x * light_size_y));
