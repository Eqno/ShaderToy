...
float
intersect_box(Ray ray, out vec3 normal, vec3 size, out vec3 ppp)
{
	float t_min = 0.0;
	float t_max = INFINITY;
	if(intersect_aabb(ray, AABB(-size, size), t_min, t_max)) {
		vec3 p = ray_at(ray, t_min) / size;

        if (abs(p.x) > abs(p.y) && abs(p.x) > abs(p.z)) {
            normal = vec3(p.x > 0. ? 1. : -1., 0., 0.);
        } else if (abs(p.y) > abs(p.x) && abs(p.y) > abs(p.z)) {
            normal = vec3(0., p.y > 0. ? 1. : -1., 0.);
        } else normal = vec3(0., 0., p.z >  0. ? 1. : -1.);
...
float
intersect(Ray ray, inout vec3 p, inout vec3 normal, out vec4 albedo, out vec3 ppp)
{
...

	{
		...
		float t = intersect_box(ray_tmp, normal_tmp, vec3(0.25, 0.5, 0.25), ppp);
		if(t < t_min) {
			t_min = t;
			p = ray_at(ray, t);
...

	{
		...
		float t = intersect_box(ray_tmp, normal_tmp, vec3(0.25, 0.25, 0.25), ppp);
		if(t < t_min) {
			t_min = t;
			p = ray_at(ray, t);
			albedo = vec4(0.7, 0.7, 0.7, 0);
...
bool
test_visibility(vec3 p1, vec3 p2, vec3 ppp)
{
	...
	vec3 n, p;
	vec4 a; // ignored
	float t_shadow = intersect(r, p, n, a, ppp);

	return t_shadow > distance(p1, p2) - 2.0 * eps;
}

vec3
pt_mis(Ray ray, out vec3 ppp)
{
	vec3 contrib = vec3(0);
	vec3 tp = vec3(1.0);
	...
	vec4 albedo;
	float t = intersect(ray, position, normal, albedo, ppp);
	...
	for(int i = 0; i < NUM_BOUNCES; i++) {
		...
		{ /* NEE */
			...
			if(G > 0.0) {
				...
				if(test_visibility(position, pos_ls, ppp)) {
		...
		{ /* brdf */
			...
			vec4 albedo_next;
			float t = intersect(ray_next, position_next, normal_next, albedo_next, ppp);
...
mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	for(int i = 0; i < NUM_SAMPLES; i++) {
		...
		ray.dir = ray_dir;
        vec3 ppp;
		vec3 c = pt_mis(ray, ppp);

        if (all(greaterThan(p, ppp.xy-.01)) && all(lessThan(p, ppp.xy+.01))) {
            c.r += 100000.;
        }
		s += c;
	}
